...
impl Map<'_> {
      ...
      len @ _ => {
            ...
            ranges.windows(2)
              ...
              .fold(BTreeSet::new(), |mut mappings, [left, right]| {
                if right.range.start > left.range.end {
                  mappings.insert(
                    RangeMapping::new(left.range.end..right.range.start, 0)
                  );
                }
        ...
        if let first = mappings.first().unwrap() {
          if first.range.start > 0 {
            mappings.insert(
              RangeMapping::new(0..first.range.start, 0)
            );
          }
        }
        #[allow(irrefutable_let_patterns)]
        if let last = mappings.last().unwrap() {
          if last.range.end < i64::MAX {
            mappings.insert(
              RangeMapping::new(last.range.end..i64::MAX, 0)
            );
          }
        }
        mappings
  ...
  pub fn map_range(&self, range: ValueRange) -> Vec<ValueRange> {
    ...
    if len == 1 {
      ...
      range_mappings.into_iter().enumerate().map(
        |(i, mapping)| match i {
  ...
  pub fn map_ranges(&self, ranges: Vec<ValueRange>) -> Vec<ValueRange> {
    ranges.into_iter().flat_map(|range| self.map_range(range)).collect()
  }
...
impl Almanac<'_> {
  ...
  pub fn map_ranges(&self, ranges: Vec<ValueRange>) -> Vec<ValueRange> {
    self.maps.iter().fold(ranges, move |acc, map| map.map_ranges(acc))
  }

  pub fn map_ranges_lowest(&self) -> i64 {
    self.map_ranges(self.seeds.clone()).iter().map(|range| range.start).min().unwrap()
  }

  pub fn map_seeds(&self) -> impl Iterator<Item = i64> + '_ {
    self.seeds.iter().flat_map(|range| range.iter().map(|seed| self.map(seed)))
  }
...
fn number_list(input: &str) -> IResult<&str, Vec<i64>> {
  terminated(
    many1(
      map_res(terminated(digit1, space0), |s: &str| s.parse::<i64>())
    ),
    opt(line_ending)
  )(input)
}
...
fn seeds(input: &str) -> IResult<&str, Vec<i64>> {
  terminated(
    preceded(ws_line(tag("seeds:")), number_list),
    multispace0
  )
  (input)
}
...
fn map_name(input: &str) -> IResult<&str, &str> {
  terminated(
    is_not(" "),
    tuple((ws_line(tag("map:")), line_ending))
  )(input)
}
...
pub fn parser_list(input: &str) -> IResult<&str, Almanac> {
  ...
  let rangelist = seeds.into_iter().map(
    |seed| ValueRange::new(seed, seed + 1)
  ).collect();
...
pub fn parser_ranges(input: &str) -> IResult<&str, Almanac> {
  let (input, seeds) = seeds(input)?;
  let (input, maps) = maps(input)?;
  let rangelist = seeds.chunks(2).map(
    |chunks| ValueRange::new(chunks[0], chunks[0] + chunks[1])
  ).collect();
  Ok((input, Almanac::new(rangelist, maps)))
}

pub fn take_between<T, I, P1, P2>(iter: I, mut mark_start: P1, mut mark_end: P2)
